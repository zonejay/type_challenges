...
type BaseType = Record<string, any> & {
  type: keyof BaseTypeMap
  enum?: any[]
  required?: unknown[]
  properties?: Record<string, unknown>
}
type HandleObject<T extends BaseType> = T['properties'] extends Record<any, any>
  ? MergeType<
      {
        [K in keyof T['properties'] as T['required'] extends unknown[]
    ...
    >
  : BaseTypeMap[T['type']]
type HandleArray<T extends BaseType & {items?: BaseType}> = T['items'] extends {type: keyof BaseType}
  ? T['items'] extends BaseType
    ? Array<JSONSchema2TS<T['items']>>
    : unknown[]
  : BaseTypeMap[T['type']]
type JSONSchema2TS<T extends BaseType> = T['type'] extends string
